# Aelaki Dictionary (Discord Sources)

All entries are sourced from Discord messages by kamidere in the Conlangs Discord Network (2024-07 to 2026-01). Dates indicate the message where the form was first attested. Entries are organized by lemma (base/citation form) where identifiable; inflected forms are listed under their lemma.

**Notation:**
- **Bold** = lemma / citation form
- `SMALL CAPS` gloss = grammatical abbreviation
- (date) = Discord message date
- Root notation: C1-C2-C3 or C1-C2-C3-C4

---

## Verbs

### Transitive Verbs (Tetraconsonantal)

<!-- kamdr (k-m-d-r, "worship, do ritual") moved to aelaki/lexicon.py -->

---

#### **honung** — "hit"
| Form | Gloss | Source |
|------|-------|--------|
| honung | "hit" (base/citation) | 2025-06-04 |

---

#### Unnamed rain verb — root unclear — "rain"
| Form | Gloss | Source |
|------|-------|--------|
| Sharokasoro | "it's raining" | 2025-06-05 |

**Notes:** Weather verbs have masculine subject and feminine object. Subject = part of sky, object = part of ground (2025-06-05).

---

### Active Intransitive Verbs (Triconsonantal)

<!-- zhrn (zh-r-n, "live") moved to aelaki/lexicon.py -->

---

#### **apafath** — "fall"
Stative/active verb used as key example for stative prefix system.

| Form | Gloss | Source |
|------|-------|--------|
| apafath | "fall" (base) | 2025-05-30 |
| Ho'apafath | "about to fall" (probable) | 2025-05-30 |
| Nü'apafath | "started falling" (inchoative) | 2025-05-30 |
| Nï'æpæfath | "stopped falling / landed" (cessative, umlauted) | 2025-05-30 |
| Ru'apafath | "resumed falling" (resumptive) | 2025-05-30 |
| Ke'apafath | "almost fell" (almost) | 2025-05-30 |

---

#### **slodon** — "shout"
| Form | Gloss | Source |
|------|-------|--------|
| Shæslodonro | "all people shouted" — MASC.COLL.3p-shout.TERM-PAST.AUD | 2025-06-05 |
| Shashaslodonro | "some people shouted" — MASC.PL.3p-shout.TERM-PAST.AUD | 2025-06-05 |

---

#### **kamamas** — "rise"
| Form | Gloss | Source |
|------|-------|--------|
| Shækamamas | "is rising" — 3p.MASC.COLL-rise.CONT | 2025-06-05 |

---

<!-- sae ("walk") moved to aelaki/lexicon.py -->

---

### Stative Intransitive Verbs

<!-- zoduk (z-d-k, "know") moved to aelaki/lexicon.py -->

---

#### **dedik** — "learn" (distinct from Nüzoduk derivation?)
| Form | Gloss | Source |
|------|-------|--------|
| Nïnïdedikoth | "I learn" (cessative continuous, 1p) | 2025-06-04 |
| Nütlixlikoth | unclear — possibly related | 2025-06-04 |

---

#### **hedek** — "become female / be female"
| Form | Gloss | Source |
|------|-------|--------|
| Nühedekoth | "I am becoming a woman (gradually, puberty)" (inchoative, 1p) | 2025-06-04 |
| Nühedekothro | "I became a woman" | 2025-06-04 |
| Nünühedekoth | "I am becoming a woman (suddenly, magic)" (inchoative continuous) | 2025-06-04 |
| Nünühedekothro | "I was becoming a woman" | 2025-06-04 |

**Notes:** Continuous (Nünü-) implies sudden/magical transformation; bare (Nü-) implies gradual change (2025-06-04).

---

#### **dhüm** — "shine, burn"
| Form | Gloss | Source |
|------|-------|--------|
| Shædhïm | "shines" — 3p.COLL.MASC-shine.COLL.GNOMIC | 2025-06-05 |
| Shædhümüm | "is shining" — 3p.COLL.MASC-shine.CONT | 2025-06-05 |
| Shædhümshë | "shone" — 3p.COLL.MASC-shine-PAST.VIS | 2025-06-05 |
| Goshædhümümüro | "has been shining" — HOD-3p.MALE.COLL-burn.CONT-AUD.PAST | 2025-06-05 |
| Gokishædhümümürom | "will shine tomorrow" — CRAST-3p.MALE.COLL-burn.CONT-AUD.FUT | 2025-06-05 |
| Gokishædhümümürum | "will shine tomorrow" (alternate spelling) | 2025-06-05 |

---

#### **bahag** — "break"
| Form | Gloss | Source |
|------|-------|--------|
| Nübahagïfro | "it broke" — INCH-break.INAN-PAST.AUD | 2025-05-31 |

---

#### **zapapaya** — "drive" (imperative form?)
| Form | Gloss | Source |
|------|-------|--------|
| Gothozapapayashë | HOD-1p.FEM.SING-drive.IMP-PAST.VIS | 2025-06-04 |

---

### Verb-like Forms (Converbs)

#### Converb prefix inventory (2025-05-10)
| Prefix | Meaning |
|--------|---------|
| ta-...-te | actions occurred simultaneously |
| ngu-...-shəlon | action done before verb |
| ngu-...-mulon | action done after verb |
| ki- | verb done to do converb (purposive) |
| be- | verb done in hopes of converb (benefactive/dative) |
| mi- | converb as sub-part of action (instrumental) |
| ha- | converb caused verb (causative) |
| sa- | if converb then verb (conditional) |
| ra- | verb in spite of converb (concessive) |
| tlu- | verb done like converb (similative) |
| aglu- | verb ends when converb happens (terminative) |
| engmo- | verb and converb share cause |
| id!o- | verb and converb done with same intent |

#### Attested converb forms
| Form | Gloss | Source |
|------|-------|--------|
| Thoyahaglok | "in the place that he is standing" (locative converb) | 2025-05-31 |
| Gothaefïfïlgom | CRAST-1PL.INC.MASC-IMPERATIVE.meet | 2025-05-26 |

---

## Nouns

<!-- bsl (b-s-l, "tree/goddess") moved to aelaki/lexicon.py -->

---

### **gar** — "body"
| Form | Gloss | Source |
|------|-------|--------|
| Gotharoth | "my body" — typical 1p pronoun (1p.FEM.SING, inalienable poss.) | 2025-05-30 |
| Gotharosh | "my body as 3p" — body as separate entity from self | 2025-05-30 |

**Notes:** -oth- is inalienable feminine singular 1p possessed marker. -osh is 3p singular feminine suffix for verb agreement (2025-05-30).

---

### **Dhihevæsh** — "sun"
| Form | Gloss | Source |
|------|-------|--------|
| Dhihevæsh | "sun" — sun.COLL-MASC.COLL.3p | 2025-06-05 |

**Notes:** The sun is collective masculine. Uses numeral Pæn (one, collective) as determiner (2025-06-05).

---

### **zahal** — root: z-h-l — "edible plant"
| Form | Gloss | Source |
|------|-------|--------|
| Zahal | "edible plant" (base) | 2025-05-30 |
| Zahalïf | "inanimate quantity of dead edible plants" | 2025-05-30 |
| Zahahalï | "a large quantity of dead edible plants" (reduplicated) | 2025-05-30 |

---

### **saromo** — root: s-r-m — "sky(?)"
Used to show noun number morphology.
| Form | Gloss | Source |
|------|-------|--------|
| Saromo | "singular" | 2025-05-02 |
| Saroromo | "plural" (reduplication of C2-V deep in stem) | 2025-05-02 |
| Sarereme | "collective" (umlaut on entire word) | 2025-05-02 |
| Saromofo | "zero" (simple suffix -fo) | 2025-05-02 |

---

### **g'nk** — root: g'-n-k — "sky fungus(?)"
| Form | Gloss | Source |
|------|-------|--------|
| G'anoko | "singular child (of sky fungus)" | 2025-05-02 |
| Kaesaromon G'okanoko | "sky fungus" (full phrase) | 2025-05-02 |

---

### **Tïdænæsh** / **Tüdënënësh** — "person/people"
| Form | Gloss | Source |
|------|-------|--------|
| Tïdænæsh | "person" — person.COLL-MASC.COLL.3p | 2025-06-05 |
| Tüdënënësh | "people (plural)" — person.PL-MASC.3p | 2025-06-05 |

---

### Kinship Terms

| Term | Meaning | Source |
|------|---------|--------|
| Hayab | "brothers" (uterine siblings) | 2025-05-09 |
| Hanak | "older siblings" | 2025-05-07 |
| Yamat | "younger siblings" | 2025-05-07 |
| Ozamba | "mother and child" (dyadic pronoun) | 2025-05-09 |
| Odapab | "father and child" (dyadic pronoun) | 2025-05-09 |
| Dzahab | "cross siblings" | 2025-05-09 |
| Dahab | "half siblings your father gave birth to when he was female" | 2025-05-07 |
| Zajal | "half siblings from mother's male phase" (always much younger) | 2025-05-07 |

**Notes:** All kinship terms can function as 1p pronouns (2025-05-09). The species is sequentially hermaphroditic: child → female → male (2025-04-15).

---

### Pseudopronouns / Dyadic Pronouns

| Form | Meaning | Source |
|------|---------|--------|
| babath | "you and me" — male inclusive we (just speaker + addressee) | 2025-05-01 |
| babatl | alternate 2p conjugation of babath | 2025-05-01 |
| Thajaba | "me and you" — masculine dyadic | 2025-05-09 |

**Notes:** Language has no true pronouns. "My body" used for physical first person; "my name" for non-physical. Dyadic kinship terms are the most common first-person references (2025-05-09). "Aelaki has depending on your definition, either dozens of pronouns or none" (2025-05-20).

---

### Proper Nouns

| Form | Meaning | Source |
|------|---------|--------|
| Honrug / Honrushag | "Henry" (Aelakized) | attested in forms |
| Sanash | "king" — king-3p.MASC.SING | 2025-05-29 |

---

## Adjectives

#### **bagadha** — "bright"
| Form | Gloss | Source |
|------|-------|--------|
| Bagadhate | "brightly" (adverb form) — bright-ADV.PRESENT | 2025-06-05 |
| Bagadhashë | "bright" (adj, past visual) | 2025-06-05 |

**Notes:** Adjectives derive from stative verbs and agree in person, gender, number. They have limited TAM: 4 evidentials × 2 tenses (2025-04-29, 2025-05-29).

---

## Adverbs

<!-- zadate (z-d-?, "lengthily") moved to aelaki/lexicon.py as zada -->

---

#### **Vadante** — "now"
| Form | Gloss | Source |
|------|-------|--------|
| Vadante | "now" — now-ADV.PRES | 2025-06-05 |

---

#### **Baltero** — "twice" (adverbial number)
| Form | Gloss | Source |
|------|-------|--------|
| Baltero | "twice (past auditory)" — two-ADV-PAST.AUD | 2025-06-05 |

---

## Numerals

### Cardinal Numbers (Base-12/60)
| Number | Form | Source |
|--------|------|--------|
| 1 | Pan | 2025-05-08 |
| 12 | Nger (dozen marker) | docs |
| 60 | Vibhi | docs |

### Number Roles (2025-05-08, 2025-05-09)
| Role | Example (1) | Formation |
|------|-------------|-----------|
| Cardinal | Pan | base |
| Ordinal | Sekon (1st, suppletive) | -on suffix |
| Partitive | Papan ("one of") | reduplication |
| Collective | Pini ("all one") | umlaut |
| Fractional | Golo ("whole") | special root |
| Adverbial | Pante ("once") | -te suffix |

**Gender agreement:** Numbers agree with nouns in gender. Even = female, odd (except 1) = male, 1 and 10 = child (2025-05-08). "Numbers are often nouns when you are talking about the number" (2025-05-08).

---

### **Pæn** — "one (collective)"
| Form | Gloss | Source |
|------|-------|--------|
| Pæn | one.COLL.ENUM.MASC — collective singular determiner for sun | 2025-06-05 |

---

## Particles and Function Words

| Form | Meaning | Source |
|------|---------|--------|
| Nono / No | topic-switch marker (switches subject before subordinate clause) | 2025-05-30 |
| Shushu | discourse particle (question particle, used with deliberative mood) | 2025-05-26, 2025-05-29 |
| Zeni | copula of identity (conjunction-like, groups things by common identity) | 2025-05-31 |
| Zi | "and" (exhaustive conjunction) | 2025-05-29 |
| Ne | topic unifier | 2025-05-29 |
| Forsash | "force" — force-MASC.SING.3p | 2025-05-29 |
| Goyakak | "early in day" — early-in-day MASC.SING.4p | 2025-05-26 |

---

## Spatial/Temporal Converb Particles

### **Snü / Snï** — locative system (2025-05-29)
| Form | Meaning | Source |
|------|---------|--------|
| Snü | "at (a location)" | 2025-05-29 |
| Snüsnü | "through (a location)" | 2025-05-29 |
| Snï | "final location / destination" | 2025-05-29 |
| Snüf | "at (inanimate/abstract)" | 2025-05-29 |
| Snïf | "final location (inanimate)" | 2025-05-29 |
| Snüfsnüf | "through (inanimate)" | 2025-05-29 |

### **Slë / Slæ** — temporal system (2025-05-29)
| Form | Meaning | Source |
|------|---------|--------|
| Slë | "at a time" | 2025-05-29 |
| Slëslë | "throughout a duration of time" | 2025-05-29 |
| Slæ | "completion time" | 2025-05-29 |
| Slëf | "at a time (inanimate)" | 2025-05-29 |
| Slæf | "completion time (inanimate)" | 2025-05-29 |

---

## Person/Agreement Morphemes

### Ki-Syllable Inventory (2025-05-09, 2025-05-30)
Person consonants: 1p = th, 2p = j, 3p = sh, 4p = unmarked

| Morpheme | Meaning |
|----------|---------|
| thu | 1p child singular |
| thuthu | 1p child plural |
| thi | 1p child inclusive/collective |
| thuf | 1p child zero |
| Sha | 1p/3p masculine marker (context-dependent) |
| Jo | 2p feminine singular |
| Jojo | 2p feminine plural |
| Je | 2p feminine collective |
| Jof | 2p feminine zero/abstract |
| Shka | "for my benefit" (benefactive) |

**Notes:** 50 total Ki-syllable morphemes. They "glob onto almost every part of speech" (2025-05-09). The system originated from classifiers in proto-Aelaki (2025-05-14):
- ïf = inanimate (counter word)
- i = child
- æ = masculine
- o = feminine

---

## Evidential/Tense Suffixes

### Full tense-evidential system (2025-05-25)

| Suffix | Meaning |
|--------|---------|
| (unmarked) | present visual |
| -sher | mythical past |
| -ro | secondhand knowledge of past / auditory |
| -shə | firsthand knowledge of past / visual |
| -shəm | inferential knowledge of past |
| -rom | secondhand knowledge of future |
| -mu | future prediction / inferential |
| -ng | promised action / jussive / imperative |
| -ya | optative / wish |
| -yam | deliberative |
| go-...-ro | hesternal auditory |
| go-...-shə | hesternal visual |
| go-...-shəm | hesternal inferential |
| go-...-ngrom | hodiernal auditory |
| go-...-nshə | hodiernal visual |
| go-...-nshəm | hodiernal inferential |
| goki-...-rom | crastinal auditory |
| goki-...-mu | crastinal inferential |

**Notes:** "Evidentiality and tense are completely fused in morphemes — there is no way of indicating past without indicating evidentiality" (2025-05-30). Hodiernal tense borders on being the "counterfactual tense" because of how much more can be expressed in it (2025-05-30).

---

## Color Terms

| Form | Color | Source |
|------|-------|--------|
| te | red | lexicon |
| ihm | green | lexicon |
| ihf | yellow | lexicon |
| a | white | lexicon |
| ŋix | black | lexicon |

---

## Seasonal Vocabulary (2025-05-31)

| Season | Element | Reasoning |
|--------|---------|-----------|
| Spring | Wood | growing plants |
| Summer | Fire | heat causes fires |
| Autumn | Earth | dying plants turn to earth |
| Winter | Water | frozen ice melts causing floods |

---

## Full Glossed Sentences

### "May the force be with you" (2025-05-29)
```
Forsash Zi Gojaroj Ne Jæsangangazya
force-MASC.SING.3p and.exhaustive >2p.FEM.POS<body-2p.FEM.SING TOPIC_UNIFIER 2p.MASC.COLL-walk.CONT-OPT.FUTURE
```

### "Long live the king" (2025-05-29)
```
Sanash Shazhærænya Zadateya
king-3p.MASC.SING 3p.MASC.SING-live.CONT-OPT.FUTURE long-ADV-OPT.FUTURE
```

### "The sun shines." (gnomic) (2025-06-05)
```
Pæn Dhihevæsh Shædhïm
one.COLL.ENUM.MASC sun.COLL-MASC.COLL.3p 3p.COLL.MASC-shine.COLL.GNOMIC
```

### "The sun is shining." (continuous) (2025-06-05)
```
Pæn Dhihevæsh Shædhümüm
one.COLL.ENUM.MASC sun.COLL-MASC.COLL.3p 3p.COLL.MASC-shine.CONT
```

### "The sun shone." (past visual) (2025-06-05)
```
Pæn Dhihevæsh Shædhümshë
one.COLL.ENUM.MASC sun.COLL-MASC.COLL.3p 3p.COLL.MASC-shine-PAST.VIS
```

### "The sun has been shining." (present perfect progressive via hodiernal) (2025-06-05)
```
Pæn Dhihevæsh Goshædhümümüro
one.COLL.ENUM.MASC sun.COLL-MALE.COLL.3p HOD-3p.MALE.COLL-burn.CONT-AUD.PAST
```

### "The sun shines brightly." (2025-06-05)
```
Pæn Dhihevæsh Shædhïm Bagadhate
one.COLL.ENUM.MASC sun.COLL-MASC.COLL.3p 3p.COLL.MASC-shine.COLL.GNOMIC bright-ADV.PRESENT
```

### "The sun is rising now." (2025-06-05)
```
Pæn Dhihevæsh Shækamamas Vadante
one.COLL.ENUM.MASC sun.COLL-MASC.COLL.3p 3p.MASC.COLL-rise.CONT now-ADV.PRES
```

### "All the people shouted." (2025-06-05)
```
Tïdænæsh Shæslodonro
person.COLL-MASC.COLL.3p MASC.COLL.3p-shout.TERM-PAST.AUD
```

### "Some of the people shouted." (2025-06-05)
```
Tüdënënësh Shashaslodonro
person.PL-MASC.3p MASC.PL.3p-shout.TERM-PAST.AUD
```

### "Many of the people shouted twice." (2025-06-05)
```
Tüdënënësh Shashaslodonro Baltero
person.PL-MASC.3p MASC.PL.3p-shout.TERM-PAST.AUD two-ADV-PAST.AUD
```

### "It's raining." (2025-06-05)
```
Sharokasoro
(weather verb — masculine subject, feminine object)
```

### Car crash sentence (2025-06-04)
```
Gothozapapayashë Vothonüdïf Sægetlæræchïfïshë Nono Hagebilidïfïshë Sakadïf
HOD-1p.FEM.SING-drive.IMP-PAST.VIS >1p.FEM.SING.POS<windshield-INAN.4p SIM.CONV-HOD-be_hit.COMP-INAN.4p-VIS.PAST TOPIC_SHIFT CAUS.CONV-HOD-fly.COMP-INAN.4p-VIS.PAST rock-4p.SING.INAN
```

### "Let's meet tomorrow morning" (2025-05-26)
```
Gothaefïfïlgom ngugoyaklon Goyakak Shushu
CRAST-1PL.INC.MASC-IMPERATIVE.meet BEFORE-early_in_day-BEFORE early_in_day-MASC.SING.4p DISCOURSE_PARTICLE
```

### Henry sentence (2025-05-31)
```
Rühüngush Shashabechekushkashrom ka Bazhashya Kühüngash nono Ëkutashashë Kühüngash Nühofuglon
```
(All masculine subjects — noted as being harder than expected because of gender agreement)

---

## Grammatical Notes Summary

### Word Order (2025-05-30, 2025-06-05)
- SVO for transitive verbs
- SV for active intransitive
- VS for stative intransitive
- SOV was original proto-Aelaki order (preserved in imperative morphology)

### Morphological Strategies
| Operation | Noun meaning | Verb meaning | Adjective/Adverb meaning |
|-----------|-------------|-------------|------------------------|
| Reduplication (C2) | Plural | Habitual (transitive) / Continuous (stative) | Comparative |
| Umlaut (whole word) | Collective | Perfect (transitive) / Gnomic (stative) | Superlative |
| f-infix | Zero number | Negative/Subjunctive | Negative |
| Unmarked | Singular | Base form | Positive |

### Alignment
Active-stative alignment. Active verbs prefix subject agreement; stative verbs suffix it. Inanimate nouns cannot be subjects of transitive or active verbs (2025-06-04).

### Name of the Language
"Aelaki is really pronounced Alaku" by non-speakers — the name itself is specifically in a collective plural form (2025-06-09).
