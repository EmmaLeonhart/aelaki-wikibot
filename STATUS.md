...
1. **Generate a Converbs section on every verb word page.** `wiki-scripts/create_word_pages.py` currently writes v1 word pages with no converb table. Extend the verb branch so every verb lemma's word page gets a "Converbs" section at the bottom: a table of Class I prefix converbs (all nine `ConverbPrefixType` entries from `aelaki/converbs.py:22`) and Class II suffix converbs (nine entries in `ConverbSuffixType` at `aelaki/converbs.py:107`), each cell built by calling the appropriate `build_prefix_converb_*` / `build_suffix_converb_*` function with a canonical 3sg.M subject. This is the operator's top priority.

2. **Audit the 4db8259d xlsx→wiki migration against current Python — as a report, not edits.** The xlsx files are gone (deleted in that commit). The audit compares `grammar/Converbs.wiki`, `grammar/TAM.wiki`, and the Proto-Aelaki section of `grammar/Aelaki.wiki` against `aelaki/*.py` and the discord corpus. Produce a discrepancy list for manual review — **do not edit either side**. Per pinned correction #1, Python and wiki are peers; converbs in particular are only partially in Python, so "Python wins" would discard real grammar. Known discrepancies: (a) `Converbs.wiki:20-29` Class I has 8 entries (`ki/ha/sa/ra/mu/ne/ta/tu`), Python now has 9 (added `ta/engmo/i?o`, still lacks `tu-`); (b) discord 2025-05-10 (`data lake/dictionary.md:67-82`) lists an older inventory with `ta-...-te` circumfix, `be-` benefactive, `mi-` instrumental, `tlu-` similative, `aglu-` terminative, `engmo-`, `id!o-` — all three sources disagree in different directions; the operator has to resolve case by case.

## Parked / unresolved (do not implement until operator decides)

- **`ta-...-te` circumfix** (`data lake/dictionary.md:70`; `aelaki/converbs.py:30`). Discord 2025-05-10 glosses `ta-...-te` as "actions occurred simultaneously". Current Python implements bare `ta-` as `ConverbPrefixType.SIMULTANEOUS` (no trailing `-te`). Unclear whether the circumfix is a separate morpheme, an older/newer form of the same thing, or a distinct aspectual distinction. Don't collapse them; flag for manual synthesis.
- **`tu-` Simulative converb prefix** (`grammar/Converbs.wiki:29`). Wiki lists a ninth Class I prefix `tu-` "as if doing". Megadoc has the Class II suffix `mutu` "as though" (`aelaki/converbs.py:117`) but no `tu-` prefix. Operator noted "Tlu Simultative is not something I'm going to be using because I decided that was phonologically illegal" — unclear if `tu-` is alive or dead. Leave wiki alone; do not add to Python.
- **Beneficiary / malefactive case-marker home** (`grammar/Dative.wiki:2`; `data lake/Aelaki_Grammar_Guide.md:2055`). `-rum` is a real role-suffix ("for the sake of"); operator clarified it is NOT a converb. It now lives in the megadoc as a "role & stance suffix" alongside the Class II converbs, and the wiki folds benefactive into Dative with negative-number → ablative/malefactive. Python has no module for role-suffixes yet. Needs a new home (likely extend `aelaki/cases.py` or create `aelaki/role_suffixes.py`), not a converb entry.
- **`be-` prefix** (`data lake/dictionary.md:74`). Discord 2025-05-10 lists `be-` as "verb done in hopes of converb (benefactive/dative)". Rejected as noise — the operator's handwriting/transcription of English abbreviations leaked into the list. Do not add to Python.
